...
export function chatPlugin(env){
 ...
 const dailyBudget=Number(env.ATOM_DAILY_BUDGET)||0;
 const plugin={name:'atom-local-chat',configureServer(server){server.middlewares.use(async(req,res,next)=>{
  ...
  // 预算门禁：达到 100% 暂停新模型调用；达到 80% 只保留已在进行的请求。
  const budget=budgetState(dailyBudget);
  if(budget.state==='exceeded')return send(503,{error:'今日模型预算已用尽，已切换到本地资料演示。看展与检索不受影响。',code:'budget_exceeded',budget});
  if(active>=3||calls>=limit)return send(429,{error:'当前模型额度或并发已达上限，请稍后再试。仍可继续看展。'});
  active++;calls++;
  const requestId='r_'+Date.now().toString(36)+Math.random().toString(36).slice(2,7);
  reserve(requestId,0);
  try{
