...
export default function App(){
 ...
 linkEventRef.current=({kind,peer,x,z,path})=>{
  ...
  if(kind==='zone-rejected'){notice('暂时过不去：'+(peer?.reason||'入口校验未通过'));return;}
  if(kind==='resumed'){const zone=peer?.zone||'town';if(zone!=='town'){setLocation(zone);if(zone==='hall')setPanel('gallery');}notice('欢迎回来，已回到你离开时的位置');return;}
 ...
 function enterHall(){if(chatId)closeChat();setPanel('gallery');setPlaceId(null);const link=linkRef.current;if(link&&link.kind==='ws'){link.requestZone('hall');return;}setLocation('hall');notice('已进入武林大会展示馆');}
