...
export default function App(){
 ...
 function openGuide(){if(chatId)closeChat();setGuideOpen(true);setGuideMessages(v=>v.length?v:[{role:'assistant',content:'我是知微，守馆人。想先从赛事结构看起，还是直接找某件作品？',workIds:[],intent:'intro'}])}
 ...
 return <div className={`app ${night?'night':''}`}>
  <header className="topbar">
   ...
   <div className="top-actions"><span className="world-status" title={"未登录的访客：可漫游、看展、阅读与分享；收藏与记忆仅存本浏览器，不与账号同步"}><i/>{peers.length?`访客 · 联机演示 ${peers.length} 人同行`:'访客 · 本地世界'}</span><button className="profile-button" onClick={()=>openPanel('settings')} aria-label="定制我的侠客"><Avatar size={34}/><span>{nickname==='初来江湖的你'?'少侠':nickname}</span><ChevronRight size={14}/></button></div>
  </header>
